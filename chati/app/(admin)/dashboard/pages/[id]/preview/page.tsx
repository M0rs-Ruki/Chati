      ...
      case "hero":
        return (
          <div className="bg-gradient-to-br from-green-500/10 to-blue-500/10 py-20 px-6 text-center rounded-lg">
            <h1 className="text-4xl md:text-6xl font-bold mb-4 text-gray-900">{component.data.title}</h1>
            <p className="text-xl text-gray-600 mb-8">{component.data.subtitle}</p>
            <Button className="bg-green-600 hover:bg-green-700">{component.data.buttonText}</Button>
          </div>
        )
      case "heading":
        ...
        return (
          <HeadingTag className={`font-bold text-gray-900 ${headingSizes[component.data.level] || "text-2xl"}`}>
            {component.data.text}
          </HeadingTag>
        )
      case "paragraph":
        return <p className="text-gray-700 leading-relaxed">{component.data.text}</p>
      case "button":
        return (
          <Button 
            className={component.data.variant === "primary" 
      ...
      case "text-image":
        return (
          <div className={`grid md:grid-cols-2 gap-8 items-center ${
            component.data.imagePosition === "left" ? "md:flex-row-reverse" : ""
          }`}>
            <div className="space-y-4">
              <h3 className="text-2xl font-bold text-gray-900">{component.data.title}</h3>
      ...
      case "feature-cards":
        return (
          <div className="space-y-6">
            <h2 className="text-3xl font-bold text-center text-gray-900">{component.data.title}</h2>
            <div className="grid md:grid-cols-3 gap-6">
              {component.data.cards?.map((card: any, idx: number) => (
                <div key={idx} className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow">
      ...
      default:
        return (
          <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg">
            <p className="text-amber-800">Unknown component type: {component.type}</p>
          </div>
        )
    }
  }
    ...
    <div className="space-y-6 p-6">
        ...
        <div className="max-w-6xl mx-auto p-8 md:p-12">
          {page.content?.blocks && page.content.blocks.length > 0 ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-gray-400 text-sm mt-2">Add components in the editor to see them here</p>
            </div>
          )}
        </div>
      </div>
      ...
      {page.metadata?.description && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <h3 className="font-semibold text-blue-900 mb-2">SEO Meta Description</h3>
